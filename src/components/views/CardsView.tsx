...
const PromptCard: React.FC<{
    ...
    return (
        <div
            className="group relative flex flex-col h-full rounded-2xl bg-[#0F1115] border border-white/[0.03] transition-all duration-500 hover:border-primary-500/30 overflow-hidden cursor-pointer"
        ...
        >
            ...
            <div className="relative p-6 flex flex-col h-full z-10 transition-transform duration-500 group-hover:translate-y-[-2px]">
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                    <h3 className="flex-1 font-bold text-lg text-white/90 leading-snug tracking-tight">
                        {prompt.title}
                    </h3>
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <button
                            ...
                            className={clsx(
                                ...
                                prompt.is_favorite
                                    ? "bg-accent-500/20 text-accent-500 shadow-[0_0_15px_rgba(245,158,11,0.2)]"
                                    : "bg-white/[0.03] text-white/30 hover:bg-white/[0.08] hover:text-white/60"
                            )}
                        >
                        ...
                        <Menu as="div" className="relative">
                            <Menu.Button
                                className="w-9 h-9 rounded-xl flex items-center justify-center bg-white/[0.03] text-white/30 hover:bg-white/[0.08] hover:text-white/60 transition-all duration-200 backdrop-blur-sm"
                            ...
                            >
                                <Menu.Items className="absolute right-0 top-full mt-2 w-44 origin-top-right rounded-2xl bg-[#16191F] border border-white/10 shadow-2xl z-50 py-2 focus:outline-none backdrop-blur-md">
                                    <Menu.Item>
                                        {({ active }) => (
                                            <button
                                    ...
                                    <Menu.Item>
                                        {({ active }) => (
                                            <button
                                                ...
                                                className={clsx(
                                                    "w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors",
                                                    active ? "bg-red-500/10 text-red-400" : "text-white/60"
                                                )}
                                            >
                ...
                {/* Tags Única Localização */}
                {prompt.tags && prompt.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-6">
                        {prompt.tags.map(tag => (
                            <span key={tag} className="text-[10px] font-bold text-primary-400/70 tracking-wider uppercase bg-primary-500/5 px-2.5 py-1 rounded-lg border border-primary-500/10">
                                {tag}
                            </span>
                        ))}
                    </div>
                    ...
                    <button
                        ...
                        className={clsx(
                            ...
                            copied
                                ...
                                : "bg-primary-500 text-white hover:shadow-[0_0_25px_rgba(var(--primary-rgb),0.3)] hover:scale-[1.05]"
                        )}
                    >
                        {copied ? (
                            <>
